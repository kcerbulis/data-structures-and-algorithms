...
const count = (target, input) => {
  ...
  let result = [];
  const summingArray = (sum, val) => sum + val;
  ...
  if(input.length){
    ...
    return(result.reduce(summingArray));
  }
  return 0;
};
...
const totalSum = (input) => {
  ...
  for (let i in input){
    let individualArraySum = input[i].reduce(arraySummer);
    eachArraySum.push(individualArraySum);
  }

  const result = eachArraySum.reduce(arraySummer)
  return result;
};

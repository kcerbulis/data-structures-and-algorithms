...
const wordsToCharList = (arr) => {
  let result = arr.split('', arr.length);
  return result;
};
...
const listFoods = (recipe) => {
  ...
  const ingredientSelector = ['G', 'oa', 'b', 'f', 'p', 'ch', 'b', 'b', 'c', 'm', 'f'];

  for (let i = 0; i < recipe.ingredients.length; i++) {
    let currentItem = recipe.ingredients[i];
    let recepieIndexStart = currentItem.indexOf(ingredientSelector[i])
    let ingredientName = currentItem.slice(recepieIndexStart);
...
const splitFoods = (recipe) => {
  ...
  const ingredientSelector = ['G', 'oa', 'b', 'f', 'p', 'ch', 'b', 'b', 'c', 'm', 'f'];
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let splitItem = gruffaloCrumble.ingredients[i].split();
    let currentItem = splitItem.toString();
    let recepieIndexStart = currentItem.indexOf(ingredientSelector[i])
    let ingredientName = currentItem.slice(recepieIndexStart);
    result.push(ingredientName);
  }

...
const findHappiness = (arr) => {
  ...
  arr.map(val =>{
    if (val.includes(':)')) result.push(val);
    return false;
  })
  return result;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  for (let i in arr){
    const numberChunk_1 = nums[i].slice(1, 4);
    const numberChunk_2 = nums[i].slice(6, 9);
    const numberChunk_3 = nums[i].slice(10, 14);

    result.push(numberChunk_1 + numberChunk_2 + numberChunk_3);
  }
  return result;
};
...
const onlyOddChars = (str) => {
  ...
  for (let i in str){
    if (counter % 2 !== 0) result.push(str.charAt(i));
    counter++;
  }
  return (result.join(''));
};
